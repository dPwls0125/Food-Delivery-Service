...
//     */
//    @org.springframework.kafka.annotation.KafkaListener(topics = "payment-events", groupId = "order-group")
//    @org.springframework.transaction.annotation.Transactional
//    public void handlePaymentFailedEvent(personal.yejin.foodDelivery.domain.payment.event.PaymentFailedEvent event) {
//        Order order = getOrderById(event.orderId());
//
//        order.changeStatus(OrderStatus.CANCELLED);
//
//        System.out.println(String.format("[OrderService] 결제 실패로 인한 주문 취소! 주문번호: %d, 사유: %s, 시도금액: %d",
...
//    @org.springframework.kafka.annotation.KafkaListener(topics = "payment-events", groupId = "order-group")
//    @org.springframework.transaction.annotation.Transactional
//    public void handlePaymentCompletedEvent(personal.yejin.foodDelivery.domain.payment.event.PaymentCompletedEvent event) {
//        Order order = getOrderById(event.orderId());
//
//        // 상태 변경 (Order 엔티티에 changeStatus 메서드가 있다고 가정, 없다면 추가 필요)
//        // 향후 빌링 내역 저장 등 후속 작업 시 event.amount(), event.paymentMethod(), event.paymentKey() 등을 적극 활용합니다.
//        order.changeStatus(OrderStatus.PAID);
//        // orderRepository.save(order); // JPA 더티 체킹 활용
//
//        System.out.println(String.format("[OrderService] 결제 완료 상태 업데이트! 주문번호: %d, 결제금액: %d, 결제수단: %s, 승인번호: %s",
